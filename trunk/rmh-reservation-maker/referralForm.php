...
<form name ="NewReferralForm" method="POST" action="referralForm.php">
    ...
    //$newParentFirstName = sanitize(get_parentfname());
            ...
            {
            $newBeginDate = sanitize($_POST['beginYear']."-".$_POST['beginMonth']."-".$_POST['beginDay']);
            $newEndDate = sanitize($_POST['endYear']."-".$_POST['endMonth']."-".$_POST['endDay']);
    ...
    else
    {
     ...
     if ($errors);
  if ($newPatientDiagnosis == "")
      echo '<p><font color="red">Please go back and enter the Patients Diagnois into the form</font></p>';
  if ($newParentLastName == "")
      echo '<p><font color="red">Please go back and enter the Parent Last Name into the form.</font></p>';
  if ($newParentFirstName == "")
      echo '<p><font color="red">Please go back and enter the Parents First Name into the form.</font></p>';
  }
        ...
        echo "The Referral Request currently has an ID of : ".$RoomReservationRequestID."<br>";
        echo "Date submitted is : " .$swDateStatusSubmitted."<br>";
            }
        
   // get_roomReservationActivityID()=$roomReservationActivityID;
   //$currentReservation->set_beginDate($newBeginDate);
   //$currentReservation->set_endDate($newEndDate);
   
   //$currentReservation = new Reservation($roomReservationActivityID, $roomReservationRequestID, $socialWorkerProfileId, $rmhStaffProfileId,
     //$familyProfileId,$activityType, $status, $swDateStatusSubmitted, $rmhDateStatusSubmitted, $newBeginDate, $newEndDate, 
     //$patientDiagnosis, $roomnote);
     //$currentReservation = insert_RoomReservationActivity( );
